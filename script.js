...
function validarNome() 
{
    try 
    {
        ...
        if (!re.test(nome)) 
        {
            //se campo inválido, retorne false para o formulário não ser enviado.
            alert("Nome inválido!!!")
            document.form.nome.focus() //focus move o cursor para o campo em questão
            return false
        }
        return true
    }
...
function validarCPF() 
{
    ...
    cpf = cpf.replace(/[\s.-]*/igm,''); //Tira traços, pontos ou caracteres especiais eventualmente digitados no CPF.
...
$(document).ready(function()
{
    ...
    {
        var cel = /^\([1-9]{2}\)9[7-9]{1}[0-9]{3}\-[0-9]{4}$/ //define parâmetros de DDD e número do celular e converte em string.

        if (!cel.test($("#telefone_cel").val())) //verifica se o fone digitado não atende a regra definida acima.
        {
            if ($("#telefone_cel").val()== '') //verifica se o campo telefone celular está vazio.
            {
...
$(document).ready(function()
{
    ...
    {
        var res = /^\([1-9]{2}\)[2-4]{1}[0-9]{3}\-[0-9]{4}$/ //define parâmetros de DDD e número do residencial e converte em string.

        if (!res.test($("#telefone_res").val())) //verifica se o fone digitado não atende a regra definida acima.
        {
            if ($("#telefone_res").val()== '') //verifica se o campo telefone celular está vazio.
            {
